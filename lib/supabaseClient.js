...
export async function getAllArtworksForJudging() {
  try {
    const { data, error } = await supabase
      .from('artworks')
      .select('id, user_name, prompt, image_url, likes, created_at')
      .order('created_at', { ascending: false })
      .limit(10000) // הגבלה גבוהה מספיק לכל היצירות
    
    if (error) {
      console.error('Error fetching artworks for judging')
      return []
    }
    
    return data || []
  } catch (e) {
    console.error('Unexpected error fetching artworks for judging')
    return []
  }
}
