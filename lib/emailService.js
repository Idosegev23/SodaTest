...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      <p>🤖 דוח נוצר אוטומטית ב-${new Date().toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem', dateStyle: 'long', timeStyle: 'short' })}</p>
      <p style="color: #999; font-size: 11px; margin-top: 12px;">דוח זה נוצר באופן אוטומטי על ידי המערכת ונשלח מדי יום ב-17:00 (שעון ישראל)</p>
    </div>
  </div>
</body>
</html>
    ...
    if (process.env.EMAIL_TO_3) {
      recipients.push(process.env.EMAIL_TO_3)
    }
...
// שליחת הודעת זוכה שבועי לכל המשתמשים
export async function sendWeeklyWinnerEmail(winner, weekDates, allUsersEmails = []) {
  try {
    const transporter = createTransporter()
...
<head>
  ...
  <style>
    ...
    .container {
      max-width: 700px;
      margin: 0 auto;
      background: linear-gradient(135deg, #0f1729 0%, #12294a 100%);
      border-radius: 24px;
      overflow: hidden;
    ...
    .winner-frame {
      background: linear-gradient(135deg, #1a3a5c 0%, #12294a 100%);
...
<body>
  <div class="container">
    <div class="header">
      <div class="crown">👑</div>
      <h1>הזוכה השבועי נבחר!</h1>
      <p class="subtitle">®ensō Gallery - שבוע ${new Date(weekDates.start).toLocaleDateString('he-IL')} - ${new Date(weekDates.end).toLocaleDateString('he-IL')}</p>
    </div>

    <div class="content">
      <div class="winner-frame">
        <div class="winner-name">🎨 ${winner.name}</div>
        <div class="winner-details">היצירה הזוכה של השבוע!</div>
        <div style="text-align: center;">
      ...
      <div class="prompt-box">
        <strong>תיאור היצירה:</strong><br/><br/>
        "${winner.prompt}"
      </div>

      <div class="prize-section">
        <h2>🎁 הפרס הזוכה</h2>
        <p><strong>סדנת קוקטיילים זוגית!</strong></p>
        <p>מזל טוב לזוכה! 🎉🍹</p>
        ${winner.email || winner.phone ? `
        <div style="margin-top: 20px; padding: 15px; background: rgba(15,23,41,0.5); border-radius: 8px; text-align: right;" dir="rtl">
          <p style="margin: 5px 0; color: #ffffff; font-size: 14px;">
            <strong>📧 ליצירת קשר:</strong> <a href="mailto:${winner.email}" style="color: #d4af37; text-decoration: none;">${winner.email}</a>
          </p>
          ${winner.phone ? `
          <p style="margin: 5px 0; color: #ffffff; font-size: 14px;">
            <strong>📱 טלפון:</strong> <span style="color: #d4af37;">${winner.phone}</span>
          </p>
          ` : ''}
        </div>
        ` : ''}
      ...
      <div style="text-align: center; margin: 40px 0;">
        <p style="color: #8892ac; font-size: 16px; margin-bottom: 20px;">
          רוצים להיות הזוכים הבאים?<br/>
          צרו יצירה ייחודית וצברו לייקים!
        </p>
        <a href="https://ensogallery.co.il" class="cta-button">
          ✨ צרו את היצירה שלכם
        </a>
      </div>
    </div>
    ...
    <div class="footer">
      <p><strong>®ensō Gallery</strong> - SodaStream Campaign</p>
      <p>🤖 הודעה נוצרה אוטומטית ב-${new Date().toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem', dateStyle: 'long', timeStyle: 'short' })}</p>
      <p style="color: #666; font-size: 12px; margin-top: 12px;">זוכה נבחר אוטומטית בכל יום ראשון על סמך מספר הלייקים מהשבוע הקודם</p>
    </div>
  </div>
</body>
</html>
    ...
    if (process.env.EMAIL_TO_2) {
      adminRecipients.push(process.env.EMAIL_TO_2)
    }
    if (process.env.EMAIL_TO_3) {
      adminRecipients.push(process.env.EMAIL_TO_3)
    }
    ...
    await transporter.sendMail({
      ...
      subject: `👑 הזוכה השבועי נבחר - ${winner.name} - ${winner.likes} לייקים`,
      html: emailHtml
    })

    // שליחה לכל המשתמשים (בקבוצות של 50 כדי לא לעבור על מגבלות)
    if (allUsersEmails && allUsersEmails.length > 0) {
      const batchSize = 50
      for (let i = 0; i < allUsersEmails.length; i += batchSize) {
        const batch = allUsersEmails.slice(i, i + batchSize)
        
        await transporter.sendMail({
          from: `"®ensō Gallery" <${process.env.EMAIL_FROM}>`,
          bcc: batch.join(', '), // שימוש ב-BCC כדי לשמור על פרטיות
          subject: `👑 הזוכה השבועי נבחר! ®ensō Gallery`,
          html: emailHtml
        })

        // המתנה קטנה בין batch ל-batch כדי לא להציף את השרת
        await new Promise(resolve => setTimeout(resolve, 1000))
      }
    }

    return { success: true, totalRecipients: adminRecipients.length + (allUsersEmails?.length || 0) }
...
<body>
  <div class="container">
    <div class="header">
      <h1>📊 דוח שבועי - ®ensō Gallery</h1>
      <p class="period">${report.week_start} - ${report.week_end}</p>
    </div>

    <div class="summary">
      <div class="stat-card">
        <div class="stat-value">${report.summary.unique_users}</div>
        <div class="stat-label">משתמשים ייחודיים</div>
      </div>
      <div class="stat-card">
        <div class="stat-value">${report.summary.total_artworks}</div>
        <div class="stat-label">יצירות נוצרו</div>
      </div>
      <div class="stat-card">
      ...
      <div class="stat-card">
        ...
        <div class="stat-label">סה"כ לייקים</div>
      </div>
      <div class="stat-card">
        <div class="stat-value">${report.summary.average_likes}</div>
        <div class="stat-label">ממוצע לייקים ליצירה</div>
      </div>
    </div>

    ${report.analytics ? `
    <div class="section" style="margin-top: 30px;">
      <h2 class="section-title">📊 נתוני Analytics - מעקב אחר תנועה באתר</h2>
      
      <div class="summary">
        <div class="stat-card">
          <div class="stat-value">${report.analytics.total_views || 0}</div>
          <div class="stat-label">צפיות באתר (Page Views)</div>
        </div>
        <div class="stat-card">
          <div class="stat-value">${report.analytics.unique_visitors || 0}</div>
          <div class="stat-label">מבקרים ייחודיים (Sessions)</div>
        </div>
      </div>
      
      ${report.analytics.devices && Object.keys(report.analytics.devices).length > 0 ? `
      <div style="background-color: #1a3a5c; border-radius: 12px; padding: 20px; margin-top: 20px; border: 1px solid #d4af37;">
        <h3 style="color: #d4af37; font-size: 18px; margin-bottom: 15px;">📱 פילוח לפי מכשירים</h3>
        <div style="color: #8892ac;">
          ${Object.entries(report.analytics.devices).map(([device, count]) => {
            const total = Object.values(report.analytics.devices).reduce((a, b) => a + b, 0)
            const percentage = ((count / total) * 100).toFixed(1)
            return `
            <div style="display: flex; justify-content: space-between; margin-bottom: 10px; align-items: center;">
              <span style="font-size: 15px;">${device === 'desktop' ? '💻 Desktop' : device === 'mobile' ? '📱 Mobile' : device === 'tablet' ? '📱 Tablet' : device}</span>
              <span style="color: #d4af37; font-weight: bold; font-size: 16px;">${count} (${percentage}%)</span>
            </div>
          `}).join('')}
        </div>
      </div>
      ...
      ${report.analytics.top_pages && report.analytics.top_pages.length > 0 ? `
      <div style="background-color: #1a3a5c; border-radius: 12px; padding: 20px; margin-top: 20px; border: 1px solid #d4af37;">
        <h3 style="color: #d4af37; font-size: 18px; margin-bottom: 15px;">📄 TOP דפים</h3>
        <div style="color: #8892ac;">
          ${report.analytics.top_pages.map((page, idx) => `
            <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
              <span style="font-size: 14px;">${idx + 1}. ${page.page || page.path || '/'}</span>
              <span style="color: #d4af37; font-weight: bold;">${page.views || page.count || 0} צפיות</span>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      ${report.analytics.referrers && report.analytics.referrers.length > 0 ? `
      <div style="background-color: #1a3a5c; border-radius: 12px; padding: 20px; margin-top: 20px; border: 1px solid #d4af37;">
        <h3 style="color: #d4af37; font-size: 18px; margin-bottom: 15px;">🔗 מקורות תנועה - מאיפה הגיעו</h3>
        <div style="color: #8892ac;">
          ${report.analytics.referrers.map((ref, idx) => `
            <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
              <span style="font-size: 14px;">${idx + 1}. ${ref.referrer || ref.name || 'Direct'}</span>
              <span style="color: #d4af37; font-weight: bold;">${ref.visits || ref.count || 0} כניסות</span>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    ${report.top_artworks.length > 0 ? `
    <div class="section">
      <h2 class="section-title">🏆 TOP 5 יצירות השבוע</h2>
      <div class="artwork-grid">
        ${report.top_artworks.map(artwork => `
          <div class="artwork-card">
            <img src="${artwork.image_url}" alt="${artwork.prompt}" />
            <div class="artwork-title">${artwork.prompt}</div>
            <div class="artwork-meta">
              <span>${artwork.user_name}</span>
              <span class="likes">❤️ ${artwork.likes}</span>
            </div>
          </div>
        `).join('')}
      </div>
    ...
    <div class="winner-section">
      <h2>👑 הזוכה השבועי</h2>
      <img src="${report.weekly_winner.artwork.image_url}" alt="${report.weekly_winner.artwork.prompt}" />
      <h3 style="color: #ffffff; margin: 15px 0;">${report.weekly_winner.artwork.user_name}</h3>
      <p style="color: #8892ac; margin-bottom: 10px;">${report.weekly_winner.artwork.prompt}</p>
      <p style="color: #d4af37; font-size: 20px; font-weight: bold;">❤️ ${report.weekly_winner.artwork.likes} לייקים</p>
      
      <div style="background: rgba(0,0,0,0.3); padding: 20px; border-radius: 12px; margin-top: 20px; text-align: right;" dir="rtl">
        <h4 style="color: #d4af37; margin: 0 0 15px 0; font-size: 18px;">📞 פרטי התקשרות לתיאום הפרס:</h4>
        <p style="color: #ffffff; margin: 8px 0; font-size: 16px;">
          <strong>📧 מייל:</strong> <a href="mailto:${report.weekly_winner.artwork.user_email}" style="color: #d4af37; text-decoration: none;">${report.weekly_winner.artwork.user_email}</a>
        </p>
        <p style="color: #ffffff; margin: 8px 0; font-size: 16px;">
          <strong>📱 טלפון:</strong> <span style="color: #d4af37;">${report.weekly_winner.artwork.user_phone || 'לא צוין'}</span>
        </p>
      </div>
    </div>
    ` : ''}
    ...
    <div class="footer">
      <p>🤖 דוח נוצר אוטומטית ב-${new Date().toLocaleString('he-IL', { timeZone: 'Asia/Jerusalem', dateStyle: 'long', timeStyle: 'short' })}</p>
      <p>®ensō Gallery - SodaStream Israel</p>
    </div>
  </div>
</body>
</html>
    ...
    const info = await transporter.sendMail({
      from: `"®ensō Gallery" <${process.env.EMAIL_FROM}>`,
      to: recipients.join(', '),
      subject: `📊 דוח שבועי - ${report.week_start} עד ${report.week_end} - ${report.summary.unique_users} משתמשים ייחודיים`,
      html: emailHtml
    })
